...
public interface LessonRepository extends JpaRepository<LessonModel, UUID> {
    ...
    List<LessonModel> findAllLessonsFromModule(@Param("moduleId") UUID moduleId);

    @Query(value = "select * from tb_lessons where module_module_id = :moduleId and lesson_id = :lessonId",
           nativeQuery = true)
    Optional<LessonModel> findLessonFromModule(@Param("moduleId") UUID moduleId, @Param("lessonId") UUID lessonId);
}
